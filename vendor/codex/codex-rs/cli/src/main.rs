...
use codex_cli::run_login_import_pool;
...
struct LoginCommand {
    ...
    /// Conton: import CodeBuddy multi-account JWT pool (JSONL) into CODEX_HOME and activate first account.
    /// Does not use OpenAI OAuth. Source can be a one-shot export of ~2000 CodeBuddy tokens.
    #[arg(long = "import-pool", value_name = "JSONL_PATH")]
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        std::process::exit(1);
                    } else if let Some(pool_path) = login_cli.import_pool {
                        run_login_import_pool(login_cli.config_overrides, pool_path).await;
