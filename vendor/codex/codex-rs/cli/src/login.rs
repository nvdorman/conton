...
use codex_login::import_pool_from_jsonl;
...
pub async fn login_with_chatgpt(
    ...
    // Conton: browser login is CodeBuddy CLI plugin auth (open authUrl + poll token),
    // not OpenAI PKCE localhost callback. Reuses the device_code_login entry of the
    // login crate (same AuthManager / auth.json storage).
    ...
    opts.issuer = "https://www.codebuddy.ai".to_string();
...
pub async fn run_login_with_device_code(
    ...
    opts.issuer = issuer_base_url.unwrap_or_else(|| "https://www.codebuddy.ai".to_string());
    ...
    match run_device_code_login(opts.clone()).await {
        ...
        Err(e) => {
            if e.kind() == std::io::ErrorKind::NotFound {
                // Conton: no OpenAI PKCE fallback — retry CodeBuddy plugin login with browser open.
                eprintln!("CodeBuddy headless login unavailable; retrying with browser open.");
                opts.open_browser = true;
                if opts.issuer.contains("openai.com") || opts.issuer.is_empty() {
                    opts.issuer = "https://www.codebuddy.ai".to_string();
                }
                match run_device_code_login(opts).await {
                    Ok(()) => {
...
/// Conton: import multi-account CodeBuddy JWT pool into `$CODEX_HOME/conton_pool.jsonl`
/// and activate the first usable account as auth.json.
pub async fn run_login_import_pool(
    cli_config_overrides: CliConfigOverrides,
    pool_path: PathBuf,
) -> ! {
    let config = load_config_or_exit(cli_config_overrides).await;
    let _login_log_guard = init_login_file_logging(&config);
    tracing::info!(path = %pool_path.display(), "importing Conton CodeBuddy pool");
    match import_pool_from_jsonl(&config.codex_home, &pool_path) {
        Ok(n) => {
            eprintln!(
                "Imported {n} CodeBuddy accounts into {} and activated the first usable JWT.",
                config.codex_home.display()
            );
            eprintln!("Pool: {}/conton_pool.jsonl", config.codex_home.display());
            std::process::exit(0);
        }
        Err(e) => {
            eprintln!("Error importing pool: {e}");
            std::process::exit(1);
        }
    }
}
