...
impl ModelClientSession {
    ...
    async fn stream_responses_api(
        ...
        let mut conton_credit_retries: u8 = 0;
        loop {
            ...
            match stream_result {
                ...
                Err(err) => {
                    // Conton: CodeBuddy credit death (429 / 11216) → rotate JWT pool and retry
                    // (bounded cascade across pool accounts).
                    let err_text = err.to_string();
                    if conton_credit_retries < 8 {
                        if let Some(manager) = auth_manager.as_ref() {
                            if manager
                                .rotate_conton_pool_if_credits_exhausted(429, &err_text)
                                .await
                                || (err_text.contains("11216")
                                    && manager
                                        .rotate_conton_pool_if_credits_exhausted(400, &err_text)
                                        .await)
                            {
                                conton_credit_retries = conton_credit_retries.saturating_add(1);
                                tracing::warn!(
                                    attempt = conton_credit_retries,
                                    "conton pool rotated after CodeBuddy credit error; retrying turn"
                                );
                                continue;
                            }
                        }
