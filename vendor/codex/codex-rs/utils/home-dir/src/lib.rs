...
/// Returns the path to the Conton/Codex configuration directory.
///
/// Conton fork defaults to `~/.conton` so sessions/auth never land in `~/.codex`.
/// Override order:
/// 1. `CODEX_HOME` (upstream env name kept for mergeability)
/// 2. `CONTON_HOME` (Conton branding alias)
/// 3. `~/.conton` (Conton default — not `~/.codex`)
///
...
///   directory exists (callers may create it).
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    let codex_home_env = std::env::var("CODEX_HOME")
        .ok()
        .filter(|val| !val.is_empty())
        .or_else(|| {
            std::env::var("CONTON_HOME")
                .ok()
                .filter(|val| !val.is_empty())
        });
...
fn find_codex_home_from_env(codex_home_env: Option<&str>) -> std::io::Result<AbsolutePathBuf> {
    // Honor CODEX_HOME / CONTON_HOME when set so users (and tests) can override.
    match codex_home_env {
        ...
        None => {
            ...
            // Conton product default (do not write sessions into ~/.codex).
            p.push(".conton");
